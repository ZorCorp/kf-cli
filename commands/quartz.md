...
- **HTML** is copied byte-identical into the garden's `quartz/static/embeds/…` tree (served at `/static/embeds/…` as `text/html`, so the deck renders in the browser and in an iframe — a raw `.html` under `content/` would be served as `application/octet-stream` and download instead). An indexed `<base>-embed.md` companion stub stays under `content/` (title from `<title>` + `tags: [html-embed]` + a link + an `<iframe>` to the static URL) so the deck shows up in Explorer/search. Opening the stub's link or the static URL directly renders the deck full-screen.
